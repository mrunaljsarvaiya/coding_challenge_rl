...
def main(cfg):
    ...
    render_interval_sec = cfg.get("render_interval", 0.2)
    ...
    save_play_gif = bool(
        cfg.get("save_play_gif", cfg.task.name == "DroneSubjectTrackEnv")
    )
    play_gif_max_frames = int(cfg.get("play_gif_max_frames", 10_000))
    play_gif_frame_stride = int(cfg.get("play_gif_frame_stride", 10))
    play_gif_path = cfg.get("play_gif_path", "play_trajectory.gif")
    if save_play_gif and cfg.task.name == "DroneSubjectTrackEnv":
        base_env.enable_play_trajectory_recording(max_frames=play_gif_max_frames)
        print(
            f"Play trajectory GIF: enabled (max {play_gif_max_frames} recorded steps, "
            f"gif stride={play_gif_frame_stride}, "
            f"fps ≈ {1.0 / (base_env.cfg.sim.dt * base_env.substeps):.1f} Hz) -> {play_gif_path}"
        )
    ...
    # Stopping condition is NOT "N episodes". The collector increments an internal
    # counter by `tensordict_out.numel()` each rollout batch (all tensor elements in
    # the batch — not plain env-step count). GIF is written when this loop exits
    # (including KeyboardInterrupt via `finally`).
    print(
        f"play.py: collecting until cumulative batch tensor elements >= total_frames={cfg.total_frames}. "
        f"Override with e.g. total_frames=500000 for a shorter run and faster GIF."
    )
    ...
    try:
        ...
        if save_play_gif and cfg.task.name == "DroneSubjectTrackEnv":
            gif_out = os.path.abspath(play_gif_path)
            base_env.save_play_trajectory_gif(
                gif_out, frame_stride=play_gif_frame_stride
            )
